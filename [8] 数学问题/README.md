...
- [数学问题](#数学问题)
  ...
  - [蓄水池算法](#蓄水池算法)
  - [约瑟夫环](#约瑟夫环)
  - [C++ STL mt19937 使用说明](#c-stl-mt19937-使用说明)
...
已知 rand_N 可以等概率生成 \[1, N\] 范围的随机数，那么：

`(rand_X() - 1) * Y + rand_Y()` ==> 可以等概率地生成 \[1, X*Y\] 范围的随机数。

并且如果想实现rand10()(当然这里只是举个例子，也可以是 rand6(), rand7())，就需要先实现 rand_N()，并且保证N大于10且是10的倍数。这样再通过rand_N() % 10 + 1 就可以得到 \[1,10\] 范围的随机数了。
